...
| `oauth2-postgres-release` | `postgres:15-alpine` | `5433:5432` | PostgreSQL（宿主机 5433，避开本地冲突）|
| `oauth2-redis-release` | `redis:alpine` | `6380:6379` | Redis（宿主机 6380，避开本地冲突）|
...
```yaml
environment:
  - OAUTH2_DB_HOST=oauth2-postgres-release         # 指向 Docker 内网的 postgres 服务名
  - OAUTH2_DB_NAME=oauth2_db
  - OAUTH2_DB_PASSWORD=postgres_secret_pass
  - OAUTH2_REDIS_HOST=oauth2-redis-release
