...
# 方式2：通过参数传递（不推荐，密码会显示在命令历史中）
.\scripts\backend\reset-account-lockout.ps1 -DbPassword "your_password"

# 重置特定用户
.\scripts\backend\reset-account-lockout.ps1 -Username admin -DbPassword "your_password"
```
