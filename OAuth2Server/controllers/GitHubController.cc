                      ...
                      if (!mappingResult.empty())
                      {
                          ...
                          db->execSqlAsync(
                            ...
                            [callbackPtr, issueTokens, userId](const drogon::orm::Result &r) {
                                std::string username = r.empty() ? "user" : r[0]["username"].as<std::string>();
                            ...
                            [callbackPtr, db, issueTokens, provider, subject, username](
                              const drogon::orm::Result &userResult
                                ...
                                db->execSqlAsync(
                                  ...
                                  [callbackPtr, issueTokens, userId, username](const drogon::orm::Result &) {
                                      ...
                                      db3->execSqlAsync(
                                        ...
                                        [issueTokens, userId, username](const drogon::orm::DrogonDbException &) {
                                            issueTokens(userId, username); // still issue tokens even if role assignment fails
                                        },
                                        userId
                                      );
                                  },
