...
DROGON_TEST(Unit_P0_OAuth2Plugin_ValidateClientScopes_RestrictsToAllowlist)
{
    ...
    // Test Case 1: Valid scopes
    {
        ...
        auto f = p.get_future();
        plugin->validateClientScopes("test-client", {"openid", "profile"}, [&](bool success, std::string error) {
    ...
    // Test Case 2: Invalid scope
    {
        std::promise<std::pair<bool, std::string>> p;
        auto f = p.get_future();
        plugin->validateClientScopes("test-client", {"openid", "admin"}, [&](bool success, std::string error) {
            p.set_value({success, error});
        });
        auto result = f.get();
