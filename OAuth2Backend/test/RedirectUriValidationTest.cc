...
DROGON_TEST(RedirectUriValidation_Atomicity)
{
    ...
    try
    {
        // Test: Ensure atomic operation - invalid redirect_uri should not consume code
        ...
        // Verify code is still available (not consumed)
        {
            ...
            LOG_INFO << "Auth code still available (not consumed by invalid redirect_uri)";
        }
...
DROGON_TEST(RedirectUriValidation_EdgeCases)
{
    ...
    try
    {
        ...
        LOG_INFO << "--- Test 1: Empty redirect URI ---";
        {
            ...
            if (f2.wait_for(std::chrono::seconds(5)) == std::future_status::timeout)
            {
                throw std::runtime_error("Consume auth code timeout");
            }
            auto result = f2.get();
...
{
    LOG_INFO << "=== Integration Test: Redirect URI Validation Security Scenarios ===";
    ...
    try
    {
        ...
        LOG_INFO << "--- Test 3: Null byte injection prevention ---";
        {
            ...
            std::string maliciousUri = "http://localhost:5173/callback\0.evil.com";
            std::promise<std::optional<OAuth2AuthCode>> p2;
            auto f2 = p2.get_future();
            storage->consumeAuthCode(
                testCode.code,
